 ...
 *
 * Modifications:
 * - print the derived groovy source when compile errors occur
 * - added grave accent section to template parser
 * - ${} section - wrap in esc call (esc must be defined in the binding) 
 * - $${} section raw output
 *
 ********************************************************************
 ********************************************************************
 ...
 */
package org.microsauce.gravy.template;
    ...
    */
    ...
    private static class GStringTemplate implements Template {
                ...
                if (c == '<') {
                    ...
                    if (c == '$') {
                        ...
                        if (c == '{') {
                            appendCharacter('{', templateExpressions, writingString);
                            writingString = true;
                            parseGStringRaw(reader, writingString, templateExpressions);
                            writingString = true;
                            continue;
                        }
                    } else if (c == '{') {
        ...
        private void parseGSstring(Reader reader, boolean writingString, StringBuilder templateExpressions) throws IOException {
            ...
            templateExpressions.append("_.esc(("); // TODO
            while (true) {
                int c = reader.read();
                if (c == -1) break;
