...
var readCache = new ScriptableMap(new ConcurrentHashMap());

/**
 * Require cache.
 */

var cacheStore = new ScriptableMap(new ConcurrentHashMap());

/**
 * Require cache.
 */

var requires = new ScriptableMap(new ConcurrentHashMap());
...
exports.clearCache = function(){
  cacheStore = new ScriptableMap(new ConcurrentHashMap());
};
...
function cache(options, compiled) {
  ...
  if (compiled && options.filename && options.cache) {
    delete readCache[options.filename]; // TODO how will ScriptableMap handle this ???
...
function read(path, options, fn) {
  ...
  if (options.cache) readCache[path] = content;
  fn(null, content);
}
...
exports.handlebars.render = function(str, options, fn) {
  var engine = requires.handlebars || (requires.handlebars = require('handlebars'));//.Handlebars);
  try {
